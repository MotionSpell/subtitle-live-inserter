...
std::unique_ptr<Pipelines::Pipeline> buildPipeline(const char *subListFilename, const int delayInSec, const int subtitleForwardTimeInSec, const char *mpdUrl);
...
void safeMain(int argc, const char* argv[]) {
	...
	auto subtitleForwardTimeInSec = argc > 3 ? atoi(argv[3]) : 0;
	auto subListFn = argc > 4 ? argv[4] : "";
	std::cerr << "Detected options:\n\turl=\"" << url << "\"\n\tdelayInSec=" << delayInSec << "\n\tsubtitleForwardTimeInSec=" << subtitleForwardTimeInSec << "\n\tsubListFn=\"" << subListFn << "\"\n";

	auto pipeline = buildPipeline(url, delayInSec, subtitleForwardTimeInSec, subListFn);
